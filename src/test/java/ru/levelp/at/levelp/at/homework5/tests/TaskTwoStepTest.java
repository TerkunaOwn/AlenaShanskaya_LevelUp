...
public class TaskTwoTestStep extends HelperBase {
    @Test
    void testUsingFilterToSendEmail() {

//---------------------1. Войти в почту
        mailAgentLogin.loginToMailAgent(properties.getProperty("ACCOUNT_LOGIN"), properties.getProperty("ACCOUNT_PASSWORD"));
...
//---------------------3.Создать новое письмо (заполнить адресата (самого себя), тему письма (должно содержать слово Тест) и тело)
        mailAgentStep.createNewMail(properties.getProperty("ACCOUNT_LOGIN"), properties.getProperty("SUBJECT_TEST"),  properties.getProperty("BODY_TEXT"));
//---------------------4.Отправить письмо
...
//---------------------7. Verify контент, адресата и тему письма (должно совпадать с пунктом 3)
        mailAgentStep.assertMailMyFolder(properties.getProperty("ACCOUNT_LOGIN"), properties.getProperty("SUBJECT_TEST"), properties.getProperty("BODY_TEXT"));
