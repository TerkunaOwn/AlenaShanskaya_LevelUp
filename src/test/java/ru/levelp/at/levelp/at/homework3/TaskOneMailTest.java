...
public class TaskOneMailTest extends LocatorsBaseTest {
    ...
    void testSendMail() {
        ...
        wait.until(ExpectedConditions.elementToBeClickable(
            By.cssSelector("[data-test-id='next-button']"))).click();

        wait.until(ExpectedConditions.elementToBeClickable(
            By.cssSelector("[name='password']"))).sendKeys(userPassword); //вводим пароль
        wait.until(ExpectedConditions.elementToBeClickable(
            By.cssSelector("[data-test-id='submit-button']"))).click();
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.cssSelector("[data-test-id='save']"))).click(); //сохраняем письмо в черновике
        wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.cssSelector("button[tabindex='700']"))).click();
        ...
        final int sendLetterCountBefore = driver.findElements(
            By.cssSelector("a.js-letter-list-item")).size();

        wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.cssSelector(buttonDraft))).click();
        wait.until(ExpectedConditions.titleIs(pageTitleDraft));
        final int draftLetterCountBefore = driver.findElements(
            By.cssSelector("a.js-letter-list-item")).size();
        ...
        String emailAddresLeter = driver.findElement(
            By.cssSelector("span[class='text--1tHKB']")).getText();
        Verify.verify(emailAddresLeter.equals(to));

        String subjectMail = driver.findElement(
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.cssSelector("[data-test-id='send']"))).click(); //нажимаем кнопку Отправить
        wait.until(ExpectedConditions.elementToBeClickable(
        ...
        wait.until(ExpectedConditions.numberOfElementsToBe(
            By.cssSelector("a.js-letter-list-item"), draftLetterCountBefore - 1));
