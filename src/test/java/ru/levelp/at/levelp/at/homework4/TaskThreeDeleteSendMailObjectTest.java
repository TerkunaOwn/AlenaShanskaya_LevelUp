...
public class TaskThreeDeleteSendMailObjectTest extends HelperBase {
    ...
    void testDeleteSendMail() {
        ...
        final int incomingLetterCountBefore = pageMailAgent.getLetterListItem();

        // 2.2. Заходим в папку Корзина и проверяем колличество писем ДО удаления нового

        pageMailAgent.openTrashMail();
        final int deleteLetterCountBefore = pageMailAgent.getLetterListItem();
        ...
        Verify.verify(pageMailAgent.getActualAddressInMailFolderMyself().equals(
            properties.getProperty("ACCOUNT_LOGIN")));
