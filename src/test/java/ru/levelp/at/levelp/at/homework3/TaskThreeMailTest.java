...
public class TaskThreeMailTest extends LocatorsBaseTest {
    ...
    void testDeleteSendMail() {
        ...
        wait.until(ExpectedConditions.elementToBeClickable(
            By.cssSelector("[name='username']"))).sendKeys(userLogin); //вводим адресс почты уже во фрейме
        wait.until(ExpectedConditions.elementToBeClickable(
            By.cssSelector("[name='username']"))).sendKeys(Keys.ENTER); //имитируем нажатие клавиши ENTER

        wait.until(ExpectedConditions.elementToBeClickable(
            By.cssSelector("[name='password']"))).sendKeys(userPassword); //вводим пароль
        wait.until(ExpectedConditions.elementToBeClickable(
            By.cssSelector("[name='password']"))).sendKeys(Keys.ENTER); //имитируем нажатие клавиши ENTER

        driver.navigate().refresh();
        wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.cssSelector("div.ph-project__account")));
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(buttonMySelf))).click();
        wait.until(ExpectedConditions.urlToBe(urlPageToMySelf));

        final int incomingLetterCountBefore = driver.findElements(By.cssSelector("a.js-letter-list-item")).size();

        // 2.2. Заходим в папку Корзина и проверяем колличество писем ДО удаления нового

        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(buttonTrash))).click();
        wait.until(ExpectedConditions.urlToBe(urlPageTrash));

        final int deleteLetterCountBefore = driver.findElements(By.cssSelector("a.js-letter-list-item")).size();
        ...
        wait.until(ExpectedConditions.elementToBeClickable(
            By.cssSelector("span[class*='button2_close']")));
        wait.until(ExpectedConditions.visibilityOfElementLocated(
        ...
        wait.until(ExpectedConditions.numberOfElementsToBe(
            By.cssSelector("a.js-letter-list-item"), incomingLetterCountBefore + 1));
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.cssSelector("span[class*='button2_delete']"))).click();
        ...
        wait.until(ExpectedConditions.numberOfElementsToBe(
            By.cssSelector("a.js-letter-list-item"), deleteLetterCountBefore + 1));
